...
- グループ分け(CSV)：`app/outputs/session_groups_matrix.csv`
...
1. アプリケーションを起動し，生成物を出力させる
    - 何度か繰り返す
2. 共起テーブルや結果(JSON)の評価値を元に生成されたグループ分けの良さを判断
  1. 共起テーブルで共起数を確認して，どの程度の共起数が共起が少なそうだといえるかを確認する
  2. `result.json`の何人の人と議論できるかの平均と分散を見て評価する
      - 平均値は，(*参加者全体の人数* / *グループ数* - 1) * *セッション数* で算出される「*1人が最大で何人と議論できるか*」と比較すれば大体の質が分かる
      - 分散は(かなり主観的だけど)体感で0.5未満だとだいぶばらつきがない感じがする
3. 必要に応じてGAのパラメータを調整
4. 1に戻る
